...
Public chrome (brand, nav, back links, footer) renders outside the dynamic holes so the prerendered shell is the real page; fallbacks are layout-stable empty regions, not "Loading…" text. Auth providers live under `/admin` only; public routes use anonymous Convex where needed so cookie auth does not block Partial Prerender shells. Illustration API routes drop `export const runtime = "nodejs"` (incompatible with Cache Components; Node remains the default). Generate / reject-and-regenerate bust `species:{slug}` via injectable `revalidateTags` → `revalidateTag(tag, "max")` on success only.
